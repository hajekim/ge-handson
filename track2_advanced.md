...
복잡한 주제를 단계별로 쪼개 웹과 사내 문서를 직접 뒤져, 출처가 달린 종합 보고서를 만들어주는 에이전트입니다. 질의 분해 → 플랜 생성 → 자율 탐색 → 반복 추론 → 보고서 완성의 **5단계 에이전틱 워크플로우**로 작동합니다.
...
- **리서치 프롬프트 복사 및 실행**:
  ...
  ```
  에이전트가 시작되면 검색 단계가 대시보드에 실시간으로 표시됩니다. 완료되면 목차와 인용 링크가 붙은 보고서, 그리고 오디오 요약본을 함께 받을 수 있습니다. 보통 **3~8분** 걸립니다.
...
비즈니스 난제에 대한 창의적인 아이디어를 설계하는 **토너먼트형 브레인스토밍 에이전트**입니다. 후보 아이디어 여러 개를 경쟁 평가 구조로 자율 생성한 뒤 비교해 최적안을 뽑아냅니다. 마케팅·제품 개발·엔지니어링 등 다양한 분야에 쓸 수 있고, 결과물은 **Gemini Notebook Enterprise로 내보내기**하면 60일 이후에도 보존됩니다.
...
- **두 번째 아이디어 세션 생성 (업무 자동화 아이디어)**:
  ...
  > [!NOTE]
  > **실습 안내 및 팁**
  > 에이전트가 여러 단계로 분석하고 소스를 직접 찾아보므로 완료까지 보통 **5~10분** 걸립니다. 세션 창을 열어둔 채로 다음 실습을 먼저 진행하고 나중에 결과를 확인해도 됩니다.
