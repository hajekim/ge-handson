...
3. 아래 3가지 신사업 기획 초안을 **각각 개별 소스로 붙여넣기**합니다.
   ...
   **[소스 A] 한국자동차 "세이프가드" 안전 커넥티드 카 에코시스템**
   ```
   한국자동차는 차량 내 센서·카메라·V2X(차량-사물 통신)를 통합한 "세이프가드" 안전 에코시스템을 2026년 출시한다. 졸음 감지·차선 이탈 경보·긴급제동 AI를 탑재해 교통사고를 최대 40% 저감하는 것이 목표다. 구독형 OTA 업데이트로 기능을 지속 확장하며, 보험사·응급서비스와 실시간 API 연동으로 사고 발생 시 자동 신고 및 구조 요청을 지원한다. 초기 국내 고급 SUV 라인업에 적용하고 2027년 글로벌 시장으로 확대한다.
   ```

   **[소스 B] 구글게임즈 "클라우드 아레나" 구독형 클라우드 게이밍 플랫폼**
   ```
   구글게임즈의 "클라우드 아레나"는 별도 하드웨어 없이 월 구독료만으로 AAA 신작 게임을 즉시 플레이할 수 있는 B2C 클라우드 스트리밍 플랫폼이다. GPU 서버 클러스터를 국내 3개 리전에 분산 배치해 레이턴시 20ms 이하를 보장하며, Gemini 기반 AI 코치가 실시간 공략·빌드 추천을 제공한다. 1인 기준 월 9,900원, 팀 플랜(최대 6인)은 월 24,900원으로 책정하며 게임사와 수익 공유 모델로 운영한다.
   ```

   **[소스 C] 에이아이 솔루션즈 "스마트센터 2.0" IoT 자율 관제 플랫폼**
   ```
   에이아이 솔루션즈의 "스마트센터 2.0"은 넥스트 테크놀로지스 스마트 오피스 프로젝트 납품 후 축적된 IoT 운영 데이터를 바탕으로 개발한 2세대 자율 관제 플랫폼이다. 공조·조명·에너지·보안 시스템을 단일 대시보드에서 AI가 자동 최적화하며 월 에너지 비용을 평균 28% 절감한다. 이상 감지 시 담당자에게 자동 알림을 발송하고, Gemini Enterprise 연동으로 자연어 질의만으로도 시스템 상태를 조회할 수 있다. SaaS 형태로 건물 100동 이상 운영사를 대상으로 2027년 상반기 출시 예정이다.
   ```

1. **기본 인포그래픽**: 대화창에 아래 프롬프트를 입력합니다.
   ```
   에이아이 솔루션즈 '스마트센터 2.0' IoT 자율 관제 플랫폼의 핵심 가치와 기대 효과를 한눈에 보여주는 인포그래픽을 생성해줘
   ```
...
2. **스케치노트 스타일**: 프롬프트 하나로 전혀 다른 디자인 언어를 지정해봅니다.
   ```
   한국자동차 '세이프가드' 커넥티드 카 에코시스템의 핵심 기능과 사용 시나리오를 스케치노트(sketch-note) 스타일로 시각화해줘
   ```
...
3. **신문 1면 스타일**: 동일 소스에서 퍼블리싱 톤의 결과물을 뽑아봅니다.
   ```
   구글게임즈 '클라우드 아레나' 서비스 출시 소식을 신문 1면 기사 인포그래픽 스타일로 만들어줘
   ```
