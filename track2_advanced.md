...
### 2) Agent Designer
코드 없이 자연어로 에이전트를 만들고 조직에 배포할 수 있는 **노코드/로우코드 에이전트 빌더**입니다. 단일 작업을 처리하는 **Custom agent**와 여러 서브에이전트를 단계별로 조율하는 **Workflow agent** 두 가지 유형을 지원합니다.

| 유형 | 특징 | 적합한 작업 |
|---|---|---|
| **Custom agent** | 자연어 프롬프트 하나로 즉시 생성 | 단일 목적의 명확한 작업 |
| **Workflow agent** | Flow Builder로 서브에이전트를 단계별 조율 | 순차적 다단계 복합 업무 |
...
## 2.6. Workflow Agent 및 ADK/CAA 통합 에이전트 개발

정밀한 비즈니스 로직과 단계별 순차 작동 방식을 구현하여 엄격한 업무 규칙을 자동화하는 **Workflow Agent** 및 ADK/CAA 심화 아키텍처 실습입니다.

### 1) Workflow Agent
복잡한 업무를 **순차적 단계(Step)**로 분해하고, 각 단계를 전담 서브에이전트가 실행하는 **멀티 에이전트 오케스트레이션** 방식입니다. Custom agent가 단일 프롬프트로 즉시 응답하는 방식이라면, Workflow agent는 Flow Builder 캔버스에서 노드를 연결해 "정보 수집 → 분석 → 보고서 작성"처럼 엄격한 순서가 필요한 업무를 자동화합니다.
