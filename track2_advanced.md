...
Google이 직접 만들고 유지 관리하는 **"Made by Google" 에이전트** 두 가지를 활용합니다. Deep Research Agent로 수백 개 소스를 자율 탐색해 종합 보고서를 생성하고, Idea Generation Agent로 토너먼트 방식의 평가를 거쳐 최적 비즈니스 아이디어를 도출합니다.

### 🔍 Deep Research Agent

복잡한 연구 주제를 하위 질문으로 분해하고, 웹 및 기업 내부 문서를 다단계로 자율 탐색해 인용 출처가 명시된 종합 보고서를 자동 생성하는 에이전트입니다. 질의 분해 → 플랜 생성 → 자율 탐색 → 반복 추론 → 보고서 완성의 **5단계 에이전틱 워크플로우**로 작동하며, Gemini 2.5 Pro의 100만 토큰 컨텍스트 윈도우를 활용해 수십 개의 문서를 동시에 추론합니다.
...
- **리서치 프롬프트 복사 및 실행**:
  ...
  ```
  에이전트가 작동하면 수집 및 심층 검색 단계가 비주얼 대시보드로 표기되며, 최종 완료 시 목차(TOC)와 풍부한 인용 링크가 매핑된 완성도 높은 마스터 보고서 및 팟캐스트 형태의 오디오 요약본이 완성됩니다. 완성까지 일반적으로 **3~8분**이 소요됩니다.
...
정밀한 비즈니스 로직과 단계별 순차 작동 방식을 구현하여 엄격한 업무 규칙을 자동화하는 **Workflow Agent** 실습입니다.
