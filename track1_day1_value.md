...
비디오 생성 도구 모드에서 아래 프롬프트를 실행해 루프 영상을 생성해 보겠습니다. 가이드 내 예시는 정적 스크린샷(.png)과 GIF로 구성되어 있습니다.
...
프롬프트를 잘 쓰면 결과물의 품질이 달라집니다. 구글이 권장하는 **C.R.A.F.T** 구조로, 원하는 답을 한 번에 얻는 법을 익혀봅니다.
...
### 2) 💡 프롬프트 작성 팁
1. **마크다운 헤더 (`#`, `##`) 활용**: 텍스트를 줄글로 나열하기보다 `# 역할 및 목표`, `# 참고 데이터` 등으로 구역을 나눠주면 프롬프트 구조가 더 명확해집니다.
2. **구분자 (`[ ]`, `<tags>`) 설계**: 지시 문장과 분석 대상 텍스트를 명확히 구분하여 AI의 오작동을 줄입니다.
3. **제약 조건은 맨 아래 배치**: LLM은 마지막에 언급한 제약 조건(글자 수 제한, 금지어 등)을 더 잘 반영합니다.
...
언제 단문 프롬프트를 버리고 C.R.A.F.T 구조화 프롬프트를 써야 하는지에 대한 기준표입니다.
